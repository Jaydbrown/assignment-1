# Assignment 1: Storage, Memory and Mappings in Solidity
...
When you access a mapping with a key, the EVM computes `keccak256(abi.encode(key, slot))` to find where the value lives in storage. Both the key and the slot number are zero-padded to 32 bytes before concatenation. For nested mappings like `mapping(address => mapping(address => uint256))`, this hashing is applied twice. The lookup always returns a value even for keys that have never been set, returning zero as the default.
